...
public class ReplicaService {
  ...
  public ReplicaService(
      String replicaId, RabbitConnectionFactory connectionFactory, Path filePath) {
    ...
    DeliverCallback readLastCallback = (consumerTag, delivery) -> {
      ...
      try {
        ...
        channel.basicPublish(
            "", QueueNames.READ_LAST_RESPONSE_QUEUE, null, lastLine.getBytes("UTF-8"));
    ...
    DeliverCallback readAllCallback = (consumerTag, delivery) -> {
      ...
      try {
        for (String line : repository.readAllLines()) {
          ...
          channel.basicPublish(
              "", QueueNames.READ_ALL_RESPONSE_QUEUE, null, line.getBytes("UTF-8"));
        }
        String eof = "__EOF__|" + replicaId;
        channel.basicPublish(
            "", QueueNames.READ_ALL_RESPONSE_QUEUE, null, eof.getBytes("UTF-8"));
