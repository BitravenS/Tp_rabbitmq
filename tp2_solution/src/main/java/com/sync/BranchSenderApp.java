...
public class BranchSenderApp {
  public static void main(String[] args) throws Exception {
    if (args.length < 8) {
      System.out.println(
          "Usage: BranchSenderApp <branchId> <dbUrl> <dbUser> <dbPass> <rabbitHost> <rabbitPort>"
              + " <rabbitUser> <rabbitPass> [--dry-run]");
      return;
    }
    ...
    if (dryRun) {
      System.out.println(
          "Dry run: would publish " + sales.size() + " sales from branch " + branchId);
    ...
    RabbitConfig rabbitConfig =
        new RabbitConfig(rabbitHost, rabbitPort, rabbitUser, rabbitPass, "sales.sync");
    SalesPublisher publisher =
        new SalesPublisher(new RabbitConnectionFactory(rabbitConfig), rabbitConfig);
